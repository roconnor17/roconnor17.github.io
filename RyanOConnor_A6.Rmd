...
In the .rmd file, you'll see my commented-out attempt at part 3. I intend to return to part 3, but it would be helpful if, in the next R lab, we could go over in class 1) how to incorporate replicate weights, and 2) what exactly the significance of the weights is. I was a little confused by the textbook explanation 

```{r}
# weights_1_80 <- read.csv("psam_h06.csv")
# weights_1_80 %>% select('SERIALNO', starts_with('WGTP')) %>%
#   select(-WGTP)
```

```{r}
# SF_pums_weighted <- left_join(SF_pums_logit_predicted,weights_1_80, by = 'SERIALNO')
# SF_pums_clean_weights <- SF_pums_weighted %>%
#   select('SERIALNO','BLD.x','MV.x','leadrisk','TEN.x','PUMA.x', starts_with('WGTP')) %>%
#   select(-WGTP)
```
```{r}
# logit_weighted <- svrepdesign(
#   data = SF_pums_logit_predicted,
#   type = "ACS",
#   repweights = SF_pums_clean_weights,
